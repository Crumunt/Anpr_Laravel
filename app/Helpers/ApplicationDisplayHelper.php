...
class ApplicationDisplayHelper
{
    ...
    public static function renderBadgeClass($label)
    {
        ...
        return match ($label) {
            ...
            "Under Review"
                => "bg-amber-100/80 text-amber-800 hover:bg-amber-200/60",
            "Approved"
            ...
            "Revoked"
                => "bg-orange-100/80 text-orange-800 hover:bg-orange-200/60",
    ...
    public static function renderCellBadge($key)
    {
        ...
        return match (true) {
            is_array($key) => ["type" => "status", "label" => $key],
            in_array($key, $roles) => ["type" => "role", "label" => $key],
            default => null,
        };
    }
    ...
    private static function getMiddleInitialsAttribute($middle_name)
    {
        ...
        $initials = array_map(
            fn($part) => strtoupper(mb_substr($part, 0, 1)) . ".",
            $parts,
        );
