...
@section('main-content')
    ...
    <div class="flex-1 md:ml-64 p-6 pt-24">
        ...
        <div
            ...
            <x-dashboard.card title="Total {{$userType}}" totalNumber="{{ $stats['total'] ?? 0 }}"
                percent="{{ $stats['total_change'] ?? 0 }}" description="Total system users" icon="users" />
            <x-dashboard.card title="Active {{$userType}}" totalNumber="{{ $stats['active'] ?? 0 }}"
                percent="{{ $stats['active_change'] ?? 0 }}" description="Currently active users" icon="check" />
            <x-dashboard.card title="Inactive {{$userType}}" totalNumber="{{ $stats['inactive'] ?? 0 }}"
                percent="{{ $stats['inactive_change'] ?? 0 }}" description="Currently inactive users" icon="ban" />
            @if($showAdminCard)
                <x-dashboard.card 
